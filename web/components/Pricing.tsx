...
export default function Pricing({ t }: { t: Copy["pricing"] }) {
  ...
  return (
    <section id="pricing" className="py-24 xl:py-32 3xl:py-40 4xl:py-52 bg-gray-50 relative overflow-hidden">
      ...
      <div className="max-w-5xl xl:max-w-6xl 3xl:max-w-8xl 4xl:max-w-11xl mx-auto px-4 sm:px-6 xl:px-8 3xl:px-12 4xl:px-28 relative">
        <div className="text-center mb-14 xl:mb-18">
          ...
          <h2 className="text-4xl xl:text-5xl 3xl:text-6xl 4xl:text-7xl font-extrabold text-gray-900 leading-tight mb-4 4xl:mb-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 xl:gap-8 3xl:gap-10 items-start">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8 xl:p-10 3xl:p-12 shadow-sm">
            <div className="mb-6">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-4xl xl:text-5xl font-black text-forest-700">{isFr ? "GRATUIT" : "FREE"}</span>
              </div>
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 w-full py-3 xl:py-4 rounded-xl bg-forest-50 border-2 border-forest-200 text-forest-700 font-bold text-sm xl:text-base hover:bg-forest-100 transition-colors mb-6"
            >
              <WaIcon />
              {t.freeCta}
            </a>
            <ul className="space-y-3 xl:space-y-4 mb-5">
              {t.freeTier.filter((item) => item.included).map((item) => (
                <li key={item.label} className="flex items-center gap-3">
                  <CheckIcon />
                  <span className="text-sm xl:text-base text-gray-700">{item.label}</span>
                </li>
              ))}
            </ul>
            {/* Upgrade hint instead of crossed-out items */}
            <div className="flex items-start gap-2 pt-4 border-t border-dashed border-gray-200">
              <ArrowIcon />
              <p className="text-xs xl:text-sm text-forest-600 font-medium leading-snug">{t.upgradeHint}</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="mb-6 relative">
              ...
              <p className="text-4xl xl:text-5xl font-extrabold text-white">{t.plusPrice}</p>
              <p className="text-sm text-green-200 mt-1">{t.plusSub}</p>
              <p className="inline-flex items-center gap-1.5 mt-3 bg-white/15 text-white text-[10px] xl:text-xs font-semibold px-3 py-1.5 rounded-full">
                {isFr ? "Moins cher qu'un appel à votre comptable." : "Cheaper than one call to your accountant."}
              </p>
            </div>
            <p className="text-green-200 text-xs xl:text-sm text-center mb-3 relative">
              {isFr ? "Paiement via MTN MoMo ou Orange Money" : "Pay via MTN MoMo or Orange Money"}
            </p>
            <a
              href={WHATSAPP_URL}
              target="_blank"
        ...
        <p className="text-center text-gray-400 text-sm xl:text-base mt-8">
          {t.finePrint}{" "}
          <span className="font-mono bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">{t.cancelWord}</span>{" "}
          {t.finePrint2}
        </p>
      </div>
    </section>
