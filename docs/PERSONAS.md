...
**Based on:** Tassi SRS v2.0 / SDD v2.0
...
| E3 | User reports zero revenue (`0`, "rien", "néant", "nothing") | Routed directly to Déclaration Néant guidance; no Acompte calculation performed | FR-TAX-3 |
...
| E8 | User switches between French and English mid-conversation | Bot honors an explicit language command and continues in the new language for subsequent messages | FR-CHAT-4, NFR-USE-1 |
...
| T7 | Subscribe + successful webhook | `ABONNEMENT` → select MTN MoMo → Campay sends `SUCCESS` webhook promptly | `payment_transactions.status='SUCCESS'`, `subscriptions` created, `users.is_plus=true`, confirmation sent | FR-SUB-1, FR-PAY-1, FR-PAY-2, FR-PAY-3 |
