...
**Version:** 2.1 (schema aligned with Milestone 1–2 implementation; Pidgin; is_plus as property; subscriptions multi-row; payment_transactions field names)
**Based on:** Tassi SRS v2.1
...
| language | VARCHAR(3) | `'fr'` \| `'en'` \| `'pcm'` (Cameroonian Pidgin English) — set at onboarding picker (FR-CHAT-4); default `'fr'` |
| annual_revenue_band | VARCHAR(16) | `'UNDER_10M'` \| `'RSI_10_50M'` \| `'OVER_50M'` \| `'UNKNOWN'` — set at onboarding, FR-TAX-5 |
| ~~is_plus~~ | ~~BOOLEAN~~ | **Not a DB column.** Derived at runtime as a Python `@property` on the ORM model: `any(s.status == 'ACTIVE' and s.period_end > now for s in user.subscriptions)`. Avoids the data-integrity risk of this flag drifting out of sync with the `subscriptions` table. |
...
|---|---|---|
| id | UUID PK | |
| user_id | UUID FK | Multiple rows per user are expected — each renewal period is a new row. "One active subscription per user" is an application-layer rule, not a DB UNIQUE constraint. |
| status | VARCHAR(12) | `'ACTIVE'` \| `'EXPIRED'` \| `'CANCELLED'` |
| period_start | TIMESTAMP | |
| period_end | TIMESTAMP | manual-renewal model, FR-SUB-2 |
| price_xaf | INTEGER | XAF amount paid; Integer (XAF has no subunit coins in circulation) |
...
| id | UUID PK | |
| user_id | UUID FK | |
| subscription_id | UUID FK | nullable — transaction is created before subscription is activated |
| campay_reference | VARCHAR(128) | Campay's reference token. Nullable until Campay responds. Must have a UNIQUE index added in a future migration (Milestone 5) to prevent double-processing of duplicate Campay callbacks. |
| operator | VARCHAR(6) | `'MTN'` \| `'ORANGE'` |
| amount_xaf | INTEGER | XAF amount; Integer (XAF has no subunit coins) |
| status | VARCHAR(8) | `'PENDING'` \| `'SUCCESS'` \| `'FAILED'` — default `'PENDING'` |
| created_at | TIMESTAMP | |
| updated_at | TIMESTAMP | updated on status transitions |
...
class TaxResult:
    ...
    cac_rate: Decimal            # stored so formatter can show UNCONFIRMED estimate
...
def calculate_rsi(
    ...
    base_acompte = (gross_revenue * rate_rsi).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    if cac_mode == "ADDITIVE":
        cac_amount = (base_acompte * cac_rate).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        ...
        cac_amount = None
        total_due = None
    return TaxResult(gross_revenue=gross_revenue, base_acompte=base_acompte,
                     cac_amount=cac_amount, cac_mode=cac_mode, cac_rate=cac_rate, total_due=total_due)
```

Implementation notes:
- Returns a typed `TaxResult` dataclass (not a plain dict) for type safety.
- All quantization uses `ROUND_HALF_UP` — rounding mode is explicit and tested.
- `cac_rate` is stored on the result so `format_tax_result()` can compute the UNCONFIRMED estimate display without re-importing config.
