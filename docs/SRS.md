...
**Version:** 2.1 (Pidgin + mid-session language detection + is_plus property design)
...
- Users have WhatsApp and basic French, English, or Cameroonian Pidgin English (PCM) literacy
...
- **FR-CHAT-4:** Support three languages: French (`fr`), English (`en`), and Cameroonian Pidgin English (`pcm`). At onboarding, present a trilingual language picker. Default to French if no selection is made.
- **FR-CHAT-7 (mid-session language detection):** After onboarding, detect the language of every inbound message and reply in that same language. Accounts for natural code-switching between `fr`, `en`, and `pcm` during a session. The onboarding language selection remains — this rule applies to mid-conversation switching only. Numeric-only input inherits the last detected language.
- **FR-CHAT-8 (typing indicator):** Before every bot reply, send the WhatsApp three-dot typing animation (mark message as read, then set typing status) so the user knows a response is being composed. Best-effort — never block or delay the actual reply if the typing indicator API call fails.
...
- **FR-TAX-3:** If sanitized revenue = 0, or input matches any zero-revenue phrase in any supported language, route to the Déclaration Néant message (free, no calculation needed) instead of FR-TAX-1/2. Recognized phrases:
  - French: `"néant"`, `"neant"`, `"rien"`
  - English: `"nothing"`, `"nada"`
  - Pidgin: `"e no get"`, `"no money"`, `"e finish"`, `"nothing dey"`, `"i no sell"`
  - Numeric: `"0"`, `"0.00"`, `"0 XAF"`, `"0 frs"` (any currency-suffixed zero)
...
- **NFR-USE-1:** Trilingual: French (`fr`), English (`en`), Cameroonian Pidgin English (`pcm`). Pidgin responses use simple conversational language — no formal grammar or technical terms. All bot message templates must have a `pcm` variant.
